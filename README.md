...
Each beat can generate a hero still and an editable screenplay block. The still controls are split into `Modify context` and `Generate still` / `Regenerate still`. Modify context previews every text and image reference sent for that beat—including screenplay, story history, character sheets, style references, and the previous still. Text can be refined per beat, and the eye control excludes an item from generation without deleting its source or attachment; those choices persist with the beat. `@Character` references resolve to that character’s active or locked sheet version. The target beat and screenplay remain authoritative over reference framing and pose when the server sends the assembled context to `fal-ai/nano-banana-2/edit` (or the text-to-image endpoint when there are no image references). Screenplay generation runs through FAL’s OpenRouter endpoint with `google/gemini-2.5-flash` by default; override it with `PRISMFLOW_STORYBOARD_SCRIPT_MODEL`. Provider credentials and model routing remain server-side.
