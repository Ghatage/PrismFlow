...
test('persists agent messages and an editable scene-linked script', () => {
  ...
  workspace.addMessage({role: 'user', text: 'Find the fox jump shot.', sceneId});
  ...
  assert.equal(project.agentWorkspace.messages[0].sceneId, sceneId);
  ...
  assert.equal(reloaded.agentWorkspace.messages[0].sceneId, sceneId);
